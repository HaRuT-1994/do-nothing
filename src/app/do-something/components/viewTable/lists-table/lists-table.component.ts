...
import { ListsModel } from 'src/app/do-something/models/listsData.interface';
import { ConfigListsService } from 'src/app/do-something/services/config-lists.service';
  ...
  filterData(search: string): void {
    if (search.length) {
      this.shownAllListss = this.allLists.filter(item => {
        for(let key in item) {
          if(key !== 'listId' && item[key] !== null && item[key].toString().includes(search)) {
            return item;
          }
        }
      })
